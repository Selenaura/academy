...
export async function recordLessonComplete(supabase, userId, courseId, lessonId) {
  ...
  // Generate spaced review items from lesson's quiz_points (async, don't block)
  generateReviewItems(supabase, userId, courseId, lessonId).catch(() => {});

  return { xp, xpResult, newBadges };
}

/**
 * Generate review items from a lesson's quiz_points.
 * Fetches the lesson JSON, extracts quiz_points, creates review_items + user_reviews.
 */
async function generateReviewItems(supabase, userId, courseId, lessonId) {
  try {
    // Fetch lesson JSON to get quiz_points
    const res = await fetch(`${typeof window !== 'undefined' ? '' : process.env.NEXT_PUBLIC_SITE_URL || ''}/courses/${courseId}/${lessonId}.json`);
    if (!res.ok) return;
    const lesson = await res.json();

    const quizPoints = lesson.quiz_points;
    if (!quizPoints || !Array.isArray(quizPoints) || quizPoints.length === 0) return;

    // Create review items (upsert to avoid duplicates)
    for (const point of quizPoints) {
      const questionText = typeof point === 'string' ? point : point.question || point;
      ...
      if (existing) {
        ...
        const { data: inserted } = await supabase
          .from('review_items')
          .insert({ course_id: courseId, lesson_id: lessonId, question_text: questionText, question_type: 'recall' })
          .select('id')
          .single();
      ...
      // Create user_review entry (schedule first review for tomorrow)
      await supabase
        .from('user_reviews')
        .upsert({
          user_id: userId,
    ...
    // Silent fail — review generation is non-critical
  }
}
