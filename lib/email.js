 ...
 */
const BREVO_LISTS = {
  lead_magnet: Number(process.env.BREVO_LIST_LEAD_MAGNET) || 3,
  waitlist_master: Number(process.env.BREVO_LIST_WAITLIST) || 4,
};
 ...
 */
export async function addBrevoContact({ email, source = 'web_form', listType = 'lead_magnet', signo = null }) {
...
/**
 * Send upsell email when a user completes (or nearly completes) a free course.
 * Pushes them to the next paid course with a discount coupon.
 */
export async function sendCourseUpsellEmail({ email, userName, completedCourseTitle, nextCourseTitle, nextCourseId, nextCoursePrice, couponCode, discountPercent }) {
  const courseUrl = `https://academy.selenaura.com/curso/${nextCourseId}`;
  const originalPriceCents = nextCoursePrice; // price in cents (e.g. 2499 = €24,99)
  const discountedPriceCents = Math.round(originalPriceCents * (100 - discountPercent) / 100);
  const discountedPrice = (discountedPriceCents / 100).toFixed(2).replace('.', ',');
  const originalPrice = (originalPriceCents / 100).toFixed(2).replace('.', ',');

  const subject = `Has completado ${completedCourseTitle} — tu siguiente paso te espera ✨`;

  const htmlContent = nurtureWrap(`
      <tr>
        <td style="padding:40px 40px 20px;">
          <h2 style="color:#ffffff;font-size:22px;font-weight:normal;margin:0 0 16px;">
            ¡Enhorabuena${userName ? `, ${userName}` : ''}! Has dado un gran paso.
          </h2>
          <p style="color:#b8b8c8;font-size:15px;line-height:1.8;margin:0 0 16px;">
            Has completado <strong style="color:#d4a853;">${completedCourseTitle}</strong> y ya tienes los fundamentos de consciencia, meditación y autoconocimiento. Ahora estás lista para el siguiente nivel.
          </p>
          <p style="color:#b8b8c8;font-size:15px;line-height:1.8;margin:0 0 16px;">
            Tu siguiente curso recomendado:
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:0 40px 20px;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#151520;border:1px solid #d4a853;border-radius:12px;">
            <tr>
              <td style="padding:24px;">
                <h3 style="color:#d4a853;font-size:18px;font-weight:normal;margin:0 0 8px;">
                  🧲 ${nextCourseTitle}
                </h3>
                <p style="color:#b8b8c8;font-size:14px;line-height:1.7;margin:0 0 16px;">
                  La "ley de atracción" con base científica real. Coherencia cardíaca, rewiring neuronal, y cómo tus estados emocionales influyen en tu biología.
                </p>
                <p style="color:#6a6a7a;font-size:13px;margin:0;">
                  <span style="text-decoration:line-through;">€${originalPrice}</span>
                  <strong style="color:#d4a853;font-size:16px;"> → €${discountedPrice}</strong>
                  <span style="color:#5BB88F;"> (${discountPercent}% dto. por ser alumna)</span>
                </p>
              </td>
            </tr>
          </table>
      ...
      <tr>
        <td style="padding:10px 40px 10px;text-align:center;">
          <a href="${courseUrl}?coupon=${couponCode}"
             style="display:inline-block;padding:16px 48px;background-color:#d4a853;color:#0a0a0f;font-size:16px;font-weight:bold;text-decoration:none;border-radius:30px;">
            Inscribirme con ${discountPercent}% de descuento →
          </a>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:10px 40px 20px;text-align:center;">
          <p style="color:#6a6a7a;font-size:12px;margin:0;">
            Usa el código <strong style="color:#b8b8c8;">${couponCode}</strong> en el checkout · Válido 7 días
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:10px 40px 20px;">
          <p style="color:#b8b8c8;font-size:14px;line-height:1.8;margin:0;text-align:center;">
            <em>"El curso de Brújula Interior me abrió los ojos. Magnetismo Consciente fue el que cambió mi vida."</em>
          </p>
        </td>
      </tr>`);
...
Has completado ${completedCourseTitle} y ya tienes los fundamentos. Tu siguiente paso recomendado:

🧲 ${nextCourseTitle}
Coherencia cardíaca, rewiring neuronal, y cómo tus estados emocionales influyen en tu biología.

Precio especial por ser alumna: €${discountedPrice} (antes €${originalPrice})
Código: ${couponCode}

Inscribirme: ${courseUrl}?coupon=${couponCode}
...
Selene Academia — academy.selenaura.com`;
  ...
  return sendEmail({
    ...
    subject,
    htmlContent: addUtmsToContent(
      htmlContent.replace('%%EMAIL%%', encodeURIComponent(email)),
      'course_upsell'
    ),
    textContent,
  });
