import { ImageResponse } from 'next/og';

export const runtime = 'edge';
export const alt = 'Selene Academia — Tu escuela de consciencia cósmica';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default function Image() {
  return new ImageResponse(
    (
      <div
        style={{
          background: 'linear-gradient(135deg, #0A0A0F 0%, #1A1A25 50%, #12121A 100%)',
          width: '100%',
          height: '100%',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
          fontFamily: 'serif',
          position: 'relative',
        }}
      >
        <div
          style={{
            position: 'absolute',
            top: 20,
            left: 20,
            right: 20,
            bottom: 20,
            border: '2px solid #C9A84C',
            borderRadius: 16,
            display: 'flex',
          }}
        />
        <div style={{ fontSize: 80, marginBottom: 20, display: 'flex' }}>☽</div>
        <div style={{ fontSize: 64, color: '#C9A84C', marginBottom: 16, display: 'flex' }}>
          Selene Academia
        </div>
        <div style={{ fontSize: 28, color: '#F0EDE4', opacity: 0.85, display: 'flex' }}>
          Tu escuela de consciencia cósmica
        </div>
        <div style={{ position: 'absolute', bottom: 50, fontSize: 20, color: '#A8A4A0', display: 'flex' }}>
          Neurociencia + Tradición Milenaria · Cursos con certificado
        </div>
      </div>
    ),
    { ...size }
  );
}
